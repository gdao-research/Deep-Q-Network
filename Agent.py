...
class Agent:
    def __init__(self, env, config):
        ...
        self.buffer = Buffer(self.C['max_size'], self.C['frame_stack'])
        self.nn = Net(self.n_state, self.n_action, self.C)
    ...
    def greedy_act(self, s):
        return self.nn.action(s)

    def perceive(self, s, a, r, d, it):
        ...
        if it >= self.C['training_start']:
            if it % self.C['train_freq'] == 0:
                ...
                #     self.lr = 5e-5
                self.lr = 1e-4
                self.nn.train(self.buffer, self.lr)
